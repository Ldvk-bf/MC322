...
public class Input {
	...
	public static String scanString(Scanner scan, String label, Predicate<String> condicao) {
		String dado;
		do {
			Print.tab(label, 0);
			dado = scan.nextLine();
		} while (!condicao.test(dado));
	...
	public static void transferirSeguro(Scanner scan, Seguradora seg) {
		...
		if (Validar.validarCPF(objCliente1.getCodigoPessoa())) {
			((ClientePF) objCliente1).getFrotaVeiculos().removerVeiculo(((SeguroPF) seguro).getVeiculo());
			((ClientePF) objCliente2).getFrotaVeiculos()
					.addVeiculo(seg.selecionarVeiculo(((SeguroPF) seguro).getVeiculo()));
		} else {
			((ClientePJ) objCliente1).getListaFrotas().remove(seg.selecionarFrota(((SeguroPJ) seguro).getFrota()));
			((ClientePJ) objCliente2).getListaFrotas().add(seg.selecionarFrota(((SeguroPJ) seguro).getFrota()));
		}
		seguro.setCliente(objCliente2.getCodigoPessoa());
	...
	public class seguroPF {
		public static SeguroPF cadastro(Scanner scan, Seguradora seg, Cliente cliente) {
			...
			return new SeguroPF(String.valueOf(Util.criarId()), Util.parseLocalDate(resultadoObj[0]),
					Util.parseLocalDate(resultadoObj[1]), cliente.getCodigoPessoa(),
					objVeiculo.getPlaca());
		}
	}
	...
	public class seguroPJ {
		public static SeguroPJ cadastro(Scanner scan, Seguradora seg, Cliente cliente) {
			...
			return new SeguroPJ(String.valueOf(Util.criarId()), Util.parseLocalDate(resultadoObj[0].toString()),
					Util.parseLocalDate(resultadoObj[1].toString()), cliente.getCodigoPessoa(), objFrota.getCode());
		}
	}
	...
	public class sinistro {
		public static Sinistro cadastro(Scanner scan, Seguradora seg, Condutor condutor, Seguro seguro) {
			...
			Sinistro novoSinistro = new Sinistro(String.valueOf(Util.criarId()), Util.parseLocalDate(dataString),
					enderecoString,
					condutor.getCpf());
			novoSinistro.setSeguro(seguro);
